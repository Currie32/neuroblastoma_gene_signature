...
PATH <- "~/GitHub/neuroblastoma_gene_signature/data/"
P_VALUE = 0.01
...
create_nomogram <- function(features_best, train, time1, time2) {
  
  # Format the best features for multivariate cox
  features_best_joined <- paste(features_best, collapse = " + ")
  
  # Run a multivariate cox
  data_best_cph <- cph(as.formula(paste("survival_train ~ ", 
                                        features_best_joined)),
                       train, surv=TRUE)
  
  # Create survival object out of summary statistics
  ddist <<- datadist(train) 
  options(datadist='ddist') 
  surv_genes_nomo <- Survival(data_best_cph)
  
  # Create nomogram
  nom.cox <- nomogram(data_best_cph,
                      fun=list(function(x)surv_genes_nomo(time1,x),
                               function(x)surv_genes_nomo(time2,x)),
                      funlabel=c("100-Day Sur. Prob.", "5000-Day Sur. Prob."),
                      lp = F)
...
# Plot nomogram using best features
nomo <- create_nomogram(features_best, train, 100, 5000)
plot(nomo)
