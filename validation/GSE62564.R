...
# Load patient data
patients <- read.csv("~/Imperial/neuroblastoma_gene_signature/data/GSE62564.csv")
patients$under_18_months <- is_under_18_months(patients)

# Load gene list
gene_list <- read.csv("~/Imperial/neuroblastoma_gene_signature/data/gene_list.csv")
genes <- colnames(patients[, (colnames(patients) %in% gene_list$external_gene_name)])

# Create survival for cox regressions
survival <- Surv(patients$event_free_survival_days, patients$event_free_survival)

# Find the combination of features that are most predictive of survival
candidate_genes <- univariate_cox_regression(genes, patients)
features_significant <- multivariate_cox_regression(candidate_genes, survival, patients)
features_best <- aic_feature_selection(features_significant, survival, patients)
model <- train_model(features_best, patients)

# Create risk scores
patients$risk_score <- predict(model, type="lp")
patients$risk_score_low <- patients$risk_score < median(patients$risk_score)

plot_roc_curve(patients)
plot_kaplan_meier(patients, survival)

# Plot hazard scores
ggforest(model, data = patients)

# Plot gene expression heatmap
pheatmap(
  patients[order(patients$risk_score),][c(features_best)][,2:length(features_best)],
  show_rownames=FALSE,
  cluster_rows=F,
  cluster_cols=F
  )
...
ggplot(patients, aes(x=risk_score_low, y=event_free_survival_days)) + 
  geom_boxplot()
...
is_under_18_months <- function(df) {
  under_18_months <- df$age_days < 548
...
multivariate_cox_regression <- function(candidate_genes, survival, df) {
  ...
  model <- coxph(
    as.formula(paste(
      "survival ~ under_18_months + male + high_risk + ",
...
plot_kaplan_meier <- function(patients, survival) {
  
  fit1 <- survfit(survival ~ risk_score_low, data=patients, na.action=na.pass)
  ggsurvplot(
    fit = fit1,
    conf.int=TRUE,
    pval = TRUE,
